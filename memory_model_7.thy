...
theory memory_model_7 imports Main  "HOL-Library.Word" HOL.String  begin
...
inductive  new_w_bytes :: " addr \<Rightarrow>  val list \<Rightarrow>  memory \<Rightarrow> memory\<Rightarrow> bool" where
  ...
  new_w_bytesL :"\<lbrakk> write_byte init_addr wrd mem = mem';
                 new_w_bytes (init_addr + 1) wlist mem' mem''\<rbrakk> \<Longrightarrow>
                 new_w_bytes init_addr (wrd# wlist) mem' mem''"    

inductive new_r_bytes :: "addr \<Rightarrow> memory \<Rightarrow>nat \<Rightarrow> val list \<Rightarrow> bool" where
  new_r_bytes0: "new_r_bytes init_addr mem 0 []" |
  new_r_bytesL: "\<lbrakk> len > 0; read_byte init_addr mem = v; 
                 new_r_bytes (init_addr +1) mem (len-1) vlist\<rbrakk> \<Longrightarrow>
                 new_r_bytes init_addr mem len (v#vlist)"
